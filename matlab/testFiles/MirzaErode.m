...
hippoBox(isnan(hippoBox)) = 0;
segmentation = hippoBox;
     ...
     for j = 1:256
         for k = 1:256
             if segmentation(i, j, k) == tmp;
                 ...
                 left(counter1,2) = j;
                 left(counter1,3) = k;
                 left(counter1,4) = segmentation(i,j,k);
             end
         end
     end
end
...
maxJ = maximum(1,2);
maxK = maximum(1,3);
hippoBox2 = NaN((maxI - minI+1), (maxJ - minJ+1), (maxK - minK+1), 'double');

for i = 1:sum_of_1
    hippoBox2(left(i,1)+1-minI, left(i,2)+1-minJ, left(i,3)+1-minK) = left(i,4);
end


imagesc(squeeze(hippoBox(10,:,:)))
colormap(gray)
figure
imagesc(squeeze(segmentation(10,:,:)))
colormap(gray)
