...
for i=1:NumberOfPatients %datasæt
    for j=1:10 %distance
        for m=1:13 %plans x,y,z
%                 tmp = DATA(i);
            index = j+10*(i-1); %For distance 1..10
            %Dataset(index,m,1) DATA{i} {m}
            
            Dataset(index,m,1) = DATA{i}{m+(13*(j-1)),1}.angularSecondMoment;
            Dataset(index,m,2) = DATA{i}{m+(13*(j-1)),1}.contrast;
            Dataset(index,m,3) = DATA{i}{m+(13*(j-1)),1}.correlation;
            Dataset(index,m,4) = DATA{i}{m+(13*(j-1)),1}.variance;
            Dataset(index,m,5) = DATA{i}{m+(13*(j-1)),1}.inverseDifferenceMoment;
            Dataset(index,m,6) = DATA{i}{m+(13*(j-1)),1}.sumAverage;
            Dataset(index,m,7) = DATA{i}{m+(13*(j-1)),1}.sumVariance;
            Dataset(index,m,8) = DATA{i}{m+(13*(j-1)),1}.sumEntropy;
            Dataset(index,m,9) = DATA{i}{m+(13*(j-1)),1}.entropy;
            Dataset(index,m,10) = DATA{i}{m+(13*(j-1)),1}.differenceVariance;
            Dataset(index,m,11) = DATA{i}{m+(13*(j-1)),1}.differenceEntropy;
            Dataset(index,m,12) = DATA{i}{m+(13*(j-1)),1}.informationMeasuresOfCorrelation1;
            Dataset(index,m,13) = DATA{i}{m+(13*(j-1)),1}.informationMeasuresOfCorrelation2;            
        end
    end
end
...
if (strcmp(var, 'simple') == 1)
    for i = 1:looping
        simplot3D(Dataset,Metrics{i}, i);
    end
elseif (strcmp(var, 'mean') == 1)
    for i = 1:looping
        meanplot3D(Dataset,Metrics{i}, i);
    end
end
