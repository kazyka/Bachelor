...
for i = start:finish
    ...
    for j = 1:length(files)
        ...
        data_3D = GLCM3DErode(files(j).name,10);
        ...
        if (labels(i) == 3)
            ...
            save(sprintf('3DErodeAD/datafileAD%1d',name1), 'data_Derivations3D');
            name1 = name1 + 1;
        else
            save(sprintf('ErodeCONTROL/ErodedatafileCO%1d',name3), 'data_Derivations');
            save(sprintf('3DErodeCONTROL/datafileAD%1d',name1), 'data_Derivations3D');
            name3 = name3 + 1;
        end
