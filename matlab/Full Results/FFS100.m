...
[DATA2D, ~] = dataloader(NumberOfPatients , 1, 'left', 'normalize', 'erode');

sortedData2D = cell(100,1);

%Dataset form , patients(100) X glcms(9 2d, 13 3d) X features(13) X
%distances(10)
dataSorted2D = forwardFeatureDataSort(DATA2D,NumberOfPatients);

%Find best and accuracy K for all 4 datasets
toD = cell(10,1);
parfor i=1:10
    toD{i} = ForwardSelection100(dataSorted2D,i,15);
    fprintf('FS done for k = %d  done \n',i)
end
save(sprintf('knnResults/Left2DNormalizedErode100'), 'toD');
